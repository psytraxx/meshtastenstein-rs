...
### Fixed
- **The app was never given the device's own private key, so it could not encrypt a private message to anyone.** The security settings sent to a paired phone carried only the public half of the device's key pair. The app needs both halves to encrypt a direct message itself, and with only the public half it treated the device as having no key pair at all — so it never asked for encrypted delivery and quietly sent every private message under the shared channel key, which the recipient then refuses. A paired phone now receives the full security settings, matching the standard firmware, which sends them in full to any client authorized by the pairing and sends an empty set only to unauthenticated ones.
- **Adding a contact from the app did not store that contact's encryption key, so private messages to them always failed.** The app shares a contact as a bundle that includes the contact's key, but the device kept only the name and identity portion and discarded the key, leaving it convinced it had no key for that contact. Private messages to a freshly added contact were therefore sent under the shared channel key, which the recipient refuses, and the app kept re-sending the contact in an attempt to establish it. Shared contacts' keys are now stored, matching the standard firmware.
- **The app showed no encryption keys at all for the device, and every private message failed as a result.** The device's own entry in the node list sent to the phone left the key field empty, even though the same entry broadcast over the air had always carried it. With no key shown for itself, the app concluded the device had no keypair and could not offer encrypted private messages to any contact, silently sending them under the shared channel key instead — which the recipient then refused. The device's own key is now included in what the phone receives.
...
- **Settings changes from the app were refused, permanently on some devices.** Admin commands carry a short-lived session key, and the device was checking that key on commands from a connected phone — but it never issues a key over Bluetooth in the first place, so there was nothing the app could send that would match. Depending on the device's state this surfaced either as a key mismatch (the app replaying a key cached from an earlier session) or, once a key aged out, as an expired session with no way to obtain a new one. Commands from a paired phone are now trusted on the strength of the pairing itself, matching the standard firmware, which likewise only authenticates admin commands arriving over the radio from other nodes. Remote commands over the mesh are still fully checked, and are now rejected as expired rather than as a mismatch when their key has genuinely aged out.

## 2026-09-13

### Fixed
- **Other nodes showed up on the phone with only a name and role — no battery or hop count.** Battery and voltage readings broadcast by other nodes were logged and then discarded, so there was nothing to show, and a node's hop distance was tracked internally but never passed on. Readings are now retained per node and delivered to the phone the way the standard firmware does it: the node list itself stays lightweight, and each node's stored battery and last known position follow separately once the initial sync finishes, shaped as if the node had just broadcast them. Hop distance is now included in the node list.
- **Nodes we had never actually heard from ourselves were reported with a signal strength of 0 dB**, which the app shows as a genuine (and very poor) reading rather than as unknown. 0 dB is a valid measurement, so it cannot also mean "no data". Whether we have a reading at all is now tracked separately from its value, and only readings taken by this device's own radio are reported — a node we know of only because a neighbour mentioned it no longer borrows that neighbour's measurement. Hop distance is likewise left unset rather than defaulting to zero, which would have read as "directly reachable".
...
- **A forged sender on the mesh could get a permanent entry in this device's node list, get shown on the phone, and even make this device transmit back at it — all before any attempt to verify the packet was genuine.** Duplicate detection, the node list, and the phone notification were all updated as soon as a packet arrived, with decryption and verification only checked afterward. Verification now happens first: a packet that fails to decrypt or decode no longer touches the node list, the phone, or the duplicate-detection state that governs future relaying. A packet that can't be verified but also isn't addressed to us is still relayed onward rather than silently dropped, since a mesh node can't tell tampering apart from an ordinary radio collision — it just no longer gets treated as trustworthy input in the meantime.
- **A private message to a contact whose encryption key this device didn't yet have was silently sent with the shared channel key instead of failing** — the phone showed the message as sent successfully, but current-generation Meshtastic firmware discards a channel-encrypted private message on arrival, so the recipient never actually saw it. Sending a private message to an unknown-key contact is now refused outright, matching how a real message failure is reported, instead of quietly downgrading to a form of encryption the recipient's own device will reject. The reverse case is fixed too: a private message received over the shared channel key, addressed directly to this device, is now recognized as one of these unsendable messages and dropped rather than accepted, matching what every other current device on the mesh does with it. A private message request that named a specific encryption key no longer silently accepts a different, previously stored key for that contact, and a private message can no longer be aimed at the entire mesh instead of a single contact. A message from a contact whose key isn't recognized yet no longer fails permanently when it could still be read as an ordinary channel message instead — only a genuine authentication failure or a truly unrecognized channel is treated as unreadable now, and this device automatically re-introduces itself so the sender can pick up its current key.
- **A node this device had heard from but not yet been introduced to could reappear after a reboot as if it had already introduced itself.** The saved node list has no room to record "we don't have a name for this node yet" separately from "this node's name happens to be blank," so a node saved in that in-between state came back from flash looking fully introduced, and this device stopped asking it for its name. Whether a node has actually introduced itself is now recorded explicitly, so a reboot can no longer make this device forget to ask.

### Fixed
- **A burst of mesh traffic arriving right as the phone connected over BLE could silently drop incoming LoRa packets.** Every inbound event — LoRa reception, BLE requests, battery updates, connect/disconnect — funnels through one shared queue ahead of the device's main event loop, and that queue was sized small enough that a flood-relay retransmission burst overlapping with the phone's config-exchange requests could fill it before it drained. The queue is now sized with real headroom for that kind of burst, matching how the standard firmware sizes its own equivalent queues generously on the same class of hardware.

### Changed
- **Log lines mentioning a packet's portnum showed only the raw number**, which meant reading the logs required memorizing which number meant what (67 is telemetry, not admin, for instance). Every such log line now shows the human-readable name alongside the number.
...
### Fixed
- **BLE connections could hang indefinitely right after the phone subscribed to notifications, with the app stuck in "connecting" and never sending its configuration request.** Every GATT write and read acknowledgment on both boards was being built but never actually sent back to the phone, due to a mistakenly unawaited response future. Most operations still worked because the phone doesn't strictly wait for the acknowledgment, but it was enough to stall some phones' internal write queue before the configuration request itself went out. Acknowledgments are now properly sent for every GATT operation.
- **The phone's configuration request could be silently dropped if it arrived while the shared event queue was briefly backed up** (e.g. right after a Bluetooth reconnect), leaving the app waiting forever with no way to know its request never arrived. Incoming phone requests are now delivered reliably instead of being dropped under queue pressure, unlike purely informational updates (connection state, battery level) where dropping a stale one is harmless.
- **The app could still hang partway through receiving its configuration — typically right after the module settings — even once the request itself was getting through.** Whether the device was allowed to notify the phone that new data was ready was tracked with a single flag covering all notifications, rather than checking whether the phone had actually subscribed to the specific data channel used for configuration delivery. If that flag was set early by an unrelated subscription (e.g. the battery level indicator), the device could believe it was clear to notify before the phone had actually subscribed to the channel that matters, and that one missed notification stalled the entire rest of the configuration silently. Readiness is now checked against the real per-channel subscription state instead of one shared flag.
- **The initial configuration handshake was cut short at a random point, leaving the app stuck partway through — most often just after the module settings.** The phone reads the handshake messages back-to-back until it gets an empty reply that marks the end. Its next read almost always arrived before the device had moved the following message into place, and an empty reply was sent in its place — so the phone stopped reading with most of the handshake still pending and waited forever for a completion marker it had already passed. The device now pulls the next pending message when a read arrives, and only reports end-of-queue when the queue is genuinely empty.
- **The initial configuration handshake was missing several message types the current app expects** (a region/preset compatibility hint, a settings category, and three newer module setting types), so parts of the app's view of the device were left unset. All of the message types the current app expects are now sent, and the queue used to deliver the handshake was sized up to fit the larger sequence.
- **The app stayed stuck on "connecting" even after successfully receiving the entire configuration.** The app's keepalive is a request/response exchange: it asks the device to confirm it is alive and waits for a reply reporting the outgoing radio queue's state. The device recognised only two kinds of message from the app and silently ignored the keepalive, so the handshake would complete and then the liveness check would never pass. The device now answers every keepalive, and also honours the variant that asks it to re-announce itself to the mesh.
- **Removing the pairing on the phone left the device permanently unable to pair again.** When a phone that has forgotten this device starts pairing afresh, the Bluetooth stack reports that the peer no longer holds the pairing — but the device ignored that report and kept its own half. Every subsequent attempt then failed the key check and was dropped by the device before it could be recovered from, so the phone could never complete pairing again without erasing the device's stored settings. The device now discards its own half of the pairing as soon as the phone signals it has lost its side, letting the new pairing proceed normally.
- **The device could tell the mesh a phone was connected before the Bluetooth link was actually secure**, briefly on every connection attempt including ones that failed to pair at all. The device now waits until the link is genuinely encrypted before treating a phone as connected, matching how the standard app's own firmware handles this.
- **A phone requesting different Bluetooth connection timing was met with silence**, which was logged internally as an error on every occurrence even though the connection kept working. The device now explicitly accepts the phone's requested timing.
- **If the phone ever attempted a pairing method other than the simple PIN display this device supports, pairing would silently stall with no explanation.** This can't happen with the standard app today, but the device now logs a clear warning instead of hanging quietly if it ever does.
- **Telling the device to disconnect from the app could leave a brief window where messages meant for that session were still being queued for it.** The device now ends the connection immediately on that request instead of waiting for the normal Bluetooth teardown.

## 2026-09-09

### Fixed
- **Reconnecting a paired phone permanently broke Bluetooth until the device was re-paired by hand.** A failure inside the BLE stack on reconnect was treated as fatal: the device erased its half of the pairing and rebooted, which also regenerated the device's Bluetooth identity key. The phone kept its now-useless pairing, so every later reconnect failed authentication in a loop that could not recover on its own. The pairing is now kept unless the phone itself rejects it, and the Bluetooth stack restarts in place — carrying the pairing across the restart — instead of rebooting the whole device.
- **The phone app could hang partway through the initial connection ("connecting…" forever, or stuck after module configs).** The whole configuration handshake was pushed into a fixed-size queue at once; a phone that disconnected mid-handshake left the rest of it queued and it was then replayed, stale, to the next connection — so the app saw a completion marker for a request it had already moved past. A large known-node list could also overflow the queue and drop the real completion marker. Leftover handshake messages are now discarded when a connection ends, the initial known-node list sent to the phone is capped so the handshake always fits (the phone learns the rest from live traffic), and a handshake for a phone that has gone away no longer stalls the mesh loop.
