...
- **Settings changes from the app stopped working once a device had been connected for a few minutes, with every command silently refused.** Admin commands carry a short-lived session key the device issues and then checks on the way back in. Checking an incoming command could itself replace that key with a freshly generated one before comparing, so once the old key aged out, the app's perfectly valid key was compared against a brand-new random one and rejected — and each automatic retry regenerated it again, so the command could never succeed. Checking a command no longer alters the key; a new one is issued only when the device sends a reply, and is refreshed at half its lifetime so the app is never handed a key that is about to expire. A command arriving with a genuinely expired key is now reported as expired rather than as a mismatch.
